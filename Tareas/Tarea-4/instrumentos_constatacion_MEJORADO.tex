...
\subsection*{4.3. Datos Históricos para Entrenamiento del Modelo}

\textbf{Contexto metodológico:}

Para implementar un modelo de Machine Learning supervisado que opere en gestión 2025, es necesario ENTRENARLO previamente con datos históricos de gestiones anteriores. Según \textcite[p. 284]{Hernandez2014}, los datos históricos constituyen fuentes secundarias válidas para investigación cuantitativa cuando permiten operacionalizar variables y validar hipótesis mediante análisis estadístico riguroso.

\vspace{0.5em}

\textbf{Datos históricos utilizados (NO forman parte de la población de estudio):}

\begin{itemize}[leftmargin=2cm]
    \item \textbf{Período de entrenamiento inicial:} Enero 2024 - Diciembre 2024 (12 meses)
    \item \textbf{Tamaño:} 9,762,041 transacciones históricas (28.83\% del total histórico)
    \item \textbf{Propósito:} Entrenar algoritmo Random Forest para aprender patrones de fraude
    \item \textbf{Justificación:} Estos datos NO son objeto de estudio, son INSUMO para construir el modelo base que luego se implementará y evaluará en gestión 2025
    \item \textbf{Nota importante:} El modelo entrenado con datos de 2024 será re-entrenado (fine-tuning) con los primeros meses de 2025 para adaptarse a nuevos patrones
\end{itemize}

\subsection*{4.4. División Temporal del Dataset de Gestión 2025 (Train-Validation-Test)}

La población de gestión 2025 (15,671,512 transacciones) se divide temporalmente para evaluar el modelo implementado:
...
\textbf{Training set} & Ene - Jun 2025 & 7,835,756 (50\%) & Re-entrenamiento del modelo con datos de 2025 (fine-tuning), actualización de patrones de fraude \\
\hline
...
\subsection*{4.5. Esquema Metodológico Completo}
...
\textbf{Fase 1: Entrenamiento inicial} & 2024 (histórico) & Entrenar Random Forest con 9.8M transacciones de 2024. \textbf{NO forma parte de la población de estudio.} \\
\hline

\rowcolor{lightblue!20}
\textbf{Fase 2: Implementación} & Ene-Jun 2025 & Re-entrenar/afinar modelo con primeros 6 meses de 2025 (7.8M transacciones de TechSport). \\
\hline

\rowcolor{lightgreen!20}
\textbf{Fase 3: Calibración} & Jul-Ago 2025 & Ajustar hiperparámetros con datos de validación (2.7M transacciones). \\
\hline

\rowcolor{orange!20}
\textbf{Fase 4: CONSTATACIÓN} & Sep-Dic 2025 & \textbf{EVALUAR modelo implementado} con test set (5.2M transacciones). Aquí se constata si el modelo cumple objetivo: F1 $\geq$ 85\%. \\
\hline
...
\textbf{Conclusión:} La \textbf{población de estudio} es gestión 2025 (15.7M transacciones de TechSport), pero el modelo se entrena inicialmente con datos históricos de 2024 (9.8M transacciones) para luego ser implementado y evaluado en 2025. La disponibilidad de la columna \texttt{is\_fraud} permite aplicar aprendizaje supervisado con Random Forest.
...
Semana 1 &
Extracción de dataset de gestión 2025 y datos históricos de 2024 desde ClickHouse &
Scripts Python con \texttt{clickhouse-driver}, \texttt{pandas.read\_sql()} &
\textbf{Dataset gestión 2025:} 15.7M transacciones de TechSport en CSV/Parquet (53 columnas). \textbf{Dataset histórico 2024 (entrenamiento):} 9.8M transacciones. Documento de estructura de datos y diccionario de variables \\
\hline
...
\textbf{Documento final:} "Instrumentos para la Constatación del Estado del Problema de Investigación" (PDF, 25-30 páginas), todos los scripts Python documentados (repositorio Git), datasets limpios y validados \\
\hline
...
Todas las actividades del cronograma utilizan \textbf{técnicas cuantitativas} exclusivamente:

\begin{itemize}[leftmargin=2cm]
    \item \textbf{Semana 1-1.5:} Análisis de datos secundarios (extracción y validación de dataset histórico)
    ...
    \item \textbf{Semana 5:} Validación estadística del dataset (división temporal, verificación de calidad)
    \item \textbf{Semana 6:} Documentación técnica y compilación de entregables
\end{itemize}
...
\begin{enumerate}[leftmargin=2cm]
    \item \textbf{Triangulación de fuentes de datos:}
    \begin{itemize}
        \item Dataset histórico 2024 (9.8M transacciones) para entrenamiento inicial
        \item Dataset gestión 2025 (15.7M transacciones) para implementación y evaluación
        \item Validación cruzada temporal: verificar que patrones de fraude detectados en 2024 persisten en 2025
    \end{itemize}
