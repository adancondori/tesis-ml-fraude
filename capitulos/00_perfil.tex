...
\subsection*{6.1. Clasificación de la Investigación según Sampieri (2018)}
\addcontentsline{toc}{subsection}{6.1. Clasificación de la Investigación según Sampieri (2018)}

\textbf{1. Enfoque de Investigación: CUANTITATIVO}

Según \textcite[p. 4]{Hernandez2018}: \textit{``El enfoque cuantitativo utiliza la recolección de datos para probar hipótesis con base en la medición numérica y el análisis estadístico''}.
...
\textbf{2. Tipo de Investigación: APLICADA}

Según \textcite[p. 29]{Hernandez2018}: \textit{``La investigación aplicada tiene como propósito resolver problemas prácticos''}.

Esta investigación resuelve un problema concreto de la empresa TechSport, genera una solución evaluable (modelo de ML), los resultados tienen utilidad práctica, y el modelo puede transferirse a organizaciones similares.

\textbf{3. Alcance de Investigación: CORRELACIONAL-EXPLICATIVO}

El estudio presenta un componente correlacional al establecer la relación entre la variable independiente (Modelo Random Forest) y la variable dependiente (Fraude transaccional). Asimismo, presenta un componente explicativo al plantear una hipótesis de relación causal: la aplicación del modelo Random Forest permite detectar fraude con F1-Score $\geq$ 85\%.

\textbf{4. Diseño de Investigación: NO EXPERIMENTAL, TRANSVERSAL, RETROSPECTIVO}

Según \textcite[p. 152]{Hernandez2018}: \textit{``En un estudio no experimental no se genera ninguna situación, sino que se observan situaciones ya existentes''}.

\begin{table}[H]
\centering
\caption{Análisis del diseño de investigación}
\label{tab:diseno_investigacion}
\begin{tabular}{|p{5cm}|p{1.5cm}|p{6cm}|}
\hline
\rowcolor{headerblue}
\textcolor{white}{\textbf{Criterio}} & \textcolor{white}{\textbf{¿Cumple?}} & \textcolor{white}{\textbf{Justificación}} \\
\hline
...
\textbf{Temporalidad: TRANSVERSAL}

Los datos se extraen \textbf{una sola vez} (snapshot de gestión 2025). La división Train/Validation/Test es una estrategia de validación de ML, NO un diseño longitudinal.

\textbf{Direccionalidad: RETROSPECTIVO}

Los datos corresponden a transacciones \textbf{ya ocurridas} (gestión 2025). Las etiquetas de fraude fueron asignadas después de los eventos.

\vspace{0.5cm}
\textbf{DISEÑO DE INVESTIGACIÓN COMPLETO:}
\begin{quote}
\textbf{No experimental, transversal, retrospectivo, correlacional-predictivo}
\end{quote}

\subsection*{6.2. Delimitación de la Investigación}
\addcontentsline{toc}{subsection}{6.2. Delimitación de la Investigación}

\textbf{Alcance Temático:}
\begin{itemize}
    \item Detección de fraude en pagos digitales mediante Machine Learning supervisado
    \item Algoritmo específico: Random Forest (ensemble learning)
    \item Tipos de fraude incluidos: tarjetas robadas o clonadas, transacciones duplicadas sospechosas, comportamientos anómalos de usuarios
\end{itemize}

\textbf{Exclusiones:}
\begin{itemize}
    \item Lavado de dinero
    \item Detección en tiempo real (streaming)
    \item Modelos de Deep Learning
    \item Análisis de imágenes o documentos de identidad
\end{itemize}

\textbf{Alcance Espacial:}
\begin{itemize}
    \item \textbf{Empresa:} TechSport Inc.
    \item \textbf{Sede:} Miami, Florida, USA
    \item \textbf{Operación:} Internacional (múltiples países)
\end{itemize}

\textbf{Alcance Temporal:}
\begin{itemize}
    \item \textbf{Período de datos:} Gestión 2025 (enero-diciembre)
    \item \textbf{Duración del estudio:} 3 meses
\end{itemize}

\textbf{Viabilidad:}
\begin{itemize}
    \item[$\checkmark$] Acceso autorizado a datos
    \item[$\checkmark$] Infraestructura computacional disponible
    \item[$\checkmark$] Plazo de 3 meses factible
    \item[$\checkmark$] Conocimiento técnico en ML
\end{itemize}

\subsection*{6.3. Población y Muestra}
\addcontentsline{toc}{subsection}{6.3. Población y Muestra}

\textbf{Población:} Totalidad de transacciones de pago procesadas por TechSport durante la gestión 2025: \textbf{15.671.512 registros}.

\textbf{Muestra:} Se utiliza \textbf{censo completo} (no muestreo) porque:
\begin{enumerate}
    \item \textbf{Disponibilidad técnica:} Los datos están almacenados y son accesibles
    \item \textbf{Capacidad computacional:} La infraestructura permite procesar 15,6M+ registros
    \item \textbf{Desbalance de clases:} El fraude representa $<$1\% de transacciones; se requiere máximo volumen
    \item \textbf{Etiquetas verificadas:} La variable \texttt{is\_fraud} está validada por el equipo de contabilidad
\end{enumerate}
...
\textbf{Confiabilidad:}
\begin{itemize}
    \item \textbf{Estabilidad temporal:} El modelo se evalúa en tres períodos temporales independientes
    \item \textbf{Intervalos de confianza:} Se calculan IC al 95\% mediante bootstrap (1000 iteraciones)
\end{itemize}

\subsection*{6.6. Análisis de los Datos}
\addcontentsline{toc}{subsection}{6.6. Análisis de los Datos}

\textbf{Análisis exploratorio de datos (EDA):} Examen de distribuciones univariadas, identificación de correlaciones entre variables, detección de outliers, y caracterización de patrones de fraude.

\textbf{Preprocesamiento y transformación:} Limpieza de datos, normalización de variables numéricas, codificación de variables categóricas, y creación de features derivadas evitando data leakage.

\textbf{Balanceo de clases:} Evaluación de estrategias SMOTE, class\_weight='balanced', o combinación híbrida.

\textbf{Entrenamiento y optimización:} Random Forest con optimización de hiperparámetros mediante GridSearchCV o RandomizedSearchCV.

\textbf{Evaluación del desempeño:} Métricas en test set temporal independiente con intervalos de confianza del 95\% mediante bootstrap (1000 muestras).

\textbf{Interpretabilidad:} Análisis de importancia de features mediante \texttt{feature\_importances\_} de Random Forest.
