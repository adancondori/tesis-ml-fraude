...
XGBoost implementa gradient boosting con optimizaciones de segunda orden y regularización avanzada \parencite{Geron2022}. A diferencia de Random Forest, XGBoost construye árboles secuencialmente, donde cada árbol corrige los errores del anterior.

La función objetivo de XGBoost incorpora pérdida y regularización:

\begin{equation}
    \mathcal{L}(\phi) = \sum_{i} l(\hat{y}_i, y_i) + \sum_{k} \Omega(f_k)
\end{equation}

donde $\Omega(f_k) = \gamma T + \frac{1}{2}\lambda \|w\|^2$ penaliza la complejidad del árbol (número de hojas $T$ y magnitud de los pesos $w$).

\textcite{Feng2024} documentan que XGBoost alcanza F1-Scores de 90-94\% en contextos de fraude transaccional, superando a Random Forest en datasets altamente desbalanceados.
...
Random Forest y XGBoost soportan class weights nativamente mediante el parámetro \texttt{class\_weight} o \texttt{scale\_pos\_weight}.
...
La revisión de la literatura científica del periodo 2020-2025 evidencia que los modelos de Machine Learning supervisados, particularmente Random Forest y XGBoost, constituyen un enfoque técnicamente validado para la detección de fraude en pagos digitales. Los estudios analizados reportan F1-Scores entre 85-94\%, superando las limitaciones de los sistemas basados en reglas estáticas en términos de adaptabilidad (capacidad de aprender nuevos patrones), precisión (menor tasa de falsos positivos/negativos) y escalabilidad (procesamiento de grandes volúmenes).
